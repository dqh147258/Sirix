...
impl App {
    ...
    async fn active_turn_id_for_thread_or_reload(
        &mut self,
        ...
        let thread = match app_server.thread_read(thread_id, /*include_turns*/ true).await {
            ...
            Err(error) => {
                tracing::warn!(
                    thread_id = %thread_id,
                    error = %error,
                    "failed to reload thread turns while resolving active turn id"
                );
                return None;
            }
        };
        ...
        match op.view() {
            AppCommandView::Interrupt => {
                let Some(turn_id) = self
                    .active_turn_id_for_thread_or_reload(app_server, thread_id)
                    .await
                else {
                    tracing::warn!(
                        thread_id = %thread_id,
                        "interrupt requested without an active turn id"
                    );
                    return Ok(true);
                };
        ...
        let editor_cmd = match external_editor::resolve_editor_command() {
            ...
            Err(external_editor::EditorError::MissingEditor) => {
                self.chat_widget
                    ...
                    "Cannot open external editor: set $VISUAL or $EDITOR before starting Sirix."
                        .to_string(),
                ));
