...
impl Screen {
    pub(crate) fn text(&mut self, c: char) {
        ...
        if width > size.cols {
            // 当终端已经窄到连一个双宽字符都放不下时，旧实现会在后面的
            // `size.cols - width` / continuation 假设上直接下溢或 unwrap。
            // 这里把这种“不可能排版”的输入降级成一次单格占位移动：先按
            // 单宽规则把越界光标折回有效行列，再清理当前位置可能残留的
            // malformed wide 状态，最后把光标推进一格，避免整个上层进程因
            // 极窄窗口而崩溃。
            self.grid_mut().col_wrap(1, false);
            let pos = self.grid().pos();
        ...
        if width == 0 {
                ...
                .is_wide()
            {
                ...
                let next_next_cell = self.grid_mut().drawing_cell_mut(next_next_pos).unwrap();
...
mod tests {
    ...
    fn screen(rows: u16, cols: u16) -> Screen {
        let mut screen = Screen::new(crate::grid::Size { rows, cols }, 0);
        screen.grid.allocate_rows();
        screen
    }

    #[test]
    fn writing_wide_char_to_single_column_screen_does_not_panic() {
        ...
        let first = screen
            .grid
            .drawing_cell(crate::grid::Pos { row: 0, col: 0 })
            .expect("single column exists");
        assert!(!first.is_wide());
        assert!(!first.is_wide_continuation());
        let _ = screen.contents_formatted();
        let _ = screen.state_formatted();
    }
}
