...
pub(crate) fn spawn_runtime(
    ...
    thread::spawn(move || {
        ...
        let _ = event_tx.send(RuntimeEvent::Result {
            stored_values: request.stored_values,
            error_text: Some("code mode runtime is disabled in this Sirix build".to_string()),
        });
    });
    Ok((command_tx, RuntimeTerminateHandle))
}
...
fn run_runtime(
    ...
    isolate_handle_tx: std_mpsc::SyncSender<RuntimeTerminateHandle>,
    runtime_command_tx: std_mpsc::Sender<RuntimeCommand>,
) {
    initialize_v8();
