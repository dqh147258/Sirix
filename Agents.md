...
- 当前项目在整体架构、部分交互形式和若干实现思路上较大程度借鉴了 Codex 的源码。
- 本地可参考的 Codex 源码路径为 `/Volumes/MacData/Data/DownloadCode/codex`。
- 当遇到某些功能实现方式不明确、边界处理不清晰或交互细节拿不准时，应优先查阅 Codex 源码中是否已有类似场景或相同处理方式，再结合当前项目实际情况落地。
- 当前 CLI 工具名称是 `sirix`。
- CLI 配置需要同时关注两个位置：用户目录下的 `.sirix`，以及工程目录下的 `.sirix`。
- 所有新增或修改的 CLI、桌面端、服务端及相关配套能力，都需要明确考虑并支持 `Windows`、`macOS`、`Linux` 三个平台，避免实现中隐含仅某一平台可用的路径、命令或行为假设。

## 代码规范相关
- 编写的代码的关键点需要写详细注释
- 用户要求的特殊修改要写详细注释
- 单文件代码建议不要超过 600 行；超过 600 行时，应结合职责边界做合理拆解与重构，避免继续堆积在单个文件中。

## Review和Git提交规则
- 在用户要求Git保存前, 要检查一下/doc/feature是否有相关代码的功能说明, 有的话调整, 没有的话补充.
- /doc/feature中文档主要说明做了什么功能, 代码在哪个文件中, 使用了什么方法实现.

## 功能线索
- 在用户准备添加新功能的时候, 可以在/doc/feature中寻找一下以前是否有类似的功能线索, 避免重复开发或者丢失关键信息.
