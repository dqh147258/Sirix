...
#[test]
fn create_tools_json_for_chat_completions_wraps_function_shape() {
    assert_eq!(
...
#[test]
fn create_tools_json_for_chat_completions_wraps_freeform_tools() {
    assert_eq!(
        create_tools_json_for_chat_completions(&[ToolSpec::Freeform(FreeformTool {
            name: "exec".to_string(),
            description: "Run a command".to_string(),
            format: FreeformToolFormat {
                r#type: "grammar".to_string(),
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "input": {
                            "type": "string",
                            "description": "The raw tool input body. Pass the exact freeform payload text without JSON wrappers inside this field."
                        }
                    },
                    "required": ["input"],
...
#[test]
fn create_tools_json_for_chat_completions_upgrades_apply_patch_to_json_function() {
    assert_eq!(
        create_tools_json_for_chat_completions(&[create_apply_patch_freeform_tool()])
            .expect("serialize chat tools"),
        vec![json!({
            "type": "function",
            "function": {
                "name": "apply_patch",
                "description": serde_json::to_value(crate::create_apply_patch_json_tool())
                    .expect("serialize apply_patch json tool")
                    .get("description")
                    .cloned()
