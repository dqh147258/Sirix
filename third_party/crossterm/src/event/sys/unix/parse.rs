...
pub(crate) fn parse_csi_cursor_position(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    ...
    // Some terminals have been observed to reply with 0-based or malformed CPR
    // coordinates such as `ESC[1;0R`. Treat those as origin instead of panicking
    // on unsigned underflow during startup.
